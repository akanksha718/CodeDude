...
async function getJWKS(jwksUrl) {
  ...
  cachedJWKS = createRemoteJWKSet(new URL(jwksUrl), {
    ...
    // 30 seconds cooldown before refreshing keys
    cacheMaxAge: 5 * 60 * 1e3
    // 5 minutes cache max age
  });
...
projectRoutes.get("/", async (c) => {
  ...
  const projectIds = await c.env.METADATA.get(`user-projects:${userId}`, "json") || [];
  if (!projectIds || projectIds.length === 0) {
    return c.json({ projects: [] });
  }
  const projects = await Promise.all(projectIds.map(async (projectId) => {
    return c.env.METADATA.get(`project:${projectId}`, "json");
  }));
  const filteredProjects = projects.filter((p) => p !== null).sort((a, b) => new Date(b.updatedAt).getTime() - new Date(a.updatedAt).getTime());
...
// .wrangler/tmp/bundle-BmmkBN/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-BmmkBN/middleware-loader.entry.ts
